function [keff,heff] = calculatekeff(ksolid,kfluid,fluidTemps,rockTemps,voidFraction,d,numRows,emissivityRocks)
...
%     VF2 = 0.26;
    ...
    for i=1:1:numRows
        ...
        hrv = (0.1952/(1+voidFraction(i)/2/(1-voidFraction(i))*(1-emissivityRocks)/emissivityRocks))*(fluidTemps(i)/100)^3;         
...
%         phi = phi2 + (phi1-phi2)*(voidFractionRocks-VF2)/(VF1-VF2);
        ...
        keff(i) = kfluid(i)*(voidFraction(i)*(1+beta*hrv*d/kfluid(i))+beta*(1-voidFraction(i))/((1/phi+hrs*d/kfluid(i))+gamma/kappa));
