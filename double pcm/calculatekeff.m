...
%         keff(i) = kFluid(i)*(voidFractionRocks*(1+0.9*hrv*d/kFluid(i))+0.9*(1-voidFractionRocks)/(1/(1/phi)+hrs*d/kFluid(i))+0.66667*(1/kappa));
		...
		heff(i)= hw0(i) + 0.0205*Re(i)*Pr(i); % Ofuchi Kunii 1965 eq (4). What is alpha_w, and why don't I have kf/dp on the RePr term?
		% <strike> alpha_w = 0.5 * alpha, where alpha = 0.0205 for cylindrical cross section </strike>
		% kew0 = 0.5kw0, per Ofuchi Kunii 1965 pg 752
		% Also alpha = 0.041 for annular cross section (Yagi and Kunii 1962 pg 101)
		% Looks like alpha_w for cylindrical CS should be 0.054 (Yagi Kunii 1965 pg 101, bottom right corner)
